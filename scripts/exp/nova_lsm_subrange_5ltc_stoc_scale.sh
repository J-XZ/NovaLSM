...
function run_bench() {
	...
	for m in ${machines[@]}; do
		while ssh -oStrictHostKeyChecking=no $m "ps -ef | grep -v grep | grep -v ssh | grep -v bash | grep -c \"cp -r\""; do
			sleep 10
			echo "waiting for $m"
		done
	done
	...
	for m in ${machines[@]}; do
		while ssh -oStrictHostKeyChecking=no $m "ps -ef | grep -v grep | grep -v ssh | grep -v bash | grep -c nova_server_main"; do
			sleep 10
			echo "waiting for $m"
		done
	done
...
# ="3"
for ltc_num_stocs_scatter_data_blocks in "3" "10"; do
